...
use super::ast::{
    AssignOp, BinOp, Const, Data, Expr, ExprKind, Function, Item, Place, Program, Stmt, StmtKind,
    VarDecl,
};
...
impl Parser {
    ...
    fn item(&mut self) -> Result<Item, CompileError> {
        ...
        if self.eat_word("fn") {
            ...
            self.expect_symbol("(")?;
            if !self.at_symbol(")") {
                return self.error("functions take no arguments, so `(` must be followed by `)`");
            }
            self.expect_symbol(")")?;
            let body = self.block()?;
            return Ok(Item::Function(Function { name, body, line }));
        }
