//! The command line front end for [`chip8::lang`].
//!
//! ```text
//! cargo run --bin c8c -- programs/leap.c8 roms/leap.ch8 [--asm programs/leap.asm]
//! ```

use std::path::PathBuf;
use std::process::ExitCode;

use chip8::lang::compile;

const USAGE: &str = "usage: c8c <source.c8> <output.ch8> [--asm <output.asm>]";
...
fn main() -> ExitCode {
    ...
    while let Some(argument) = arguments.next() {
        match argument.as_str() {
            ...
            other if other.starts_with('-') => {
                eprintln!("c8c: unknown option {other}\n{USAGE}");
                return ExitCode::from(2);
            }
    ...
    let source = match std::fs::read_to_string(source_path) {
        Ok(source) => source,
        Err(error) => {
            eprintln!("c8c: {}: {error}", source_path.display());
    ...
    if let Some(assembly_path) = &assembly_path {
        if let Err(error) = std::fs::write(assembly_path, &compiled.assembly) {
            eprintln!("c8c: {}: {error}", assembly_path.display());
            return ExitCode::FAILURE;
        }
    ...
    if let Err(error) = std::fs::write(rom_path, &compiled.rom) {
        eprintln!("c8c: {}: {error}", rom_path.display());
        return ExitCode::FAILURE;
    }
