...
fn the_tutorial_program_draws_six_glyphs_and_stops() {
    ...
    chip8.step_many(500).expect("no fault");

    // Every glyph is five rows tall, on rows 13 to 17 and nowhere else.
    for row in 0..32 {
        let lit = (0..64).filter(|&column| chip8.pixel(column, row)).count();
    ...
    assert!(chip8.pixel(17, 13), "the A should start at column 17");
    assert!(!chip8.pixel(46, 13), "nothing should be drawn past the 3");
}
