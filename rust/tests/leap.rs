...
use chip8::cpu::{Chip8, SCREEN_WIDTH};
...
impl Game {
    ...
    fn frames(&mut self, frames: u32) {
        for _ in 0..frames {
            self.chip8.step_many(60).expect("no fault");
    ...
    fn x(&self) -> u8 {
        self.chip8.register(X)
    }

    fn state(&self) -> u8 {
        self.chip8.register(STATE)
    }

    fn row(&self, y: usize) -> Vec<bool> {
        (0..SCREEN_WIDTH).map(|x| self.chip8.pixel(x, y)).collect()
    }

    fn lit(&self) -> usize {
        self.chip8.framebuffer().iter().filter(|&&on| on).count()
    }
