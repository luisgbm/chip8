...
fn every_program_loads_and_runs_without_an_unexpected_fault() {
    for program in BUILTIN_PROGRAMS {
        let mut chip8 = Chip8::with_seed(0xC0FF_EE00_1234_5678);
        chip8
            .load(program.rom)
            .unwrap_or_else(|error| panic!("{}: {error}", program.name));

        assert_eq!(chip8.program(), program.rom);

        // A couple of seconds of emulated time.
        for _ in 0..120 {
            if let Err(fault) = chip8.step_many(program.cycles_per_frame) {
                assert!(
...
fn the_ibm_logo_shows_up_on_screen() {
    ...
    chip8.load(program.rom).expect("it fits");
    chip8.step_many(40).expect("no fault");

    let lit = chip8.framebuffer().iter().filter(|&&pixel| pixel).count();
...
fn pong_draws_its_court_and_waits_for_a_player() {
    ...
    let mut chip8 = Chip8::with_seed(0xDEAD_BEEF_CAFE_F00D);
    chip8.load(program.rom).expect("it fits");

    for _ in 0..60 {
        chip8.step_many(program.cycles_per_frame).expect("no fault");
        chip8.tick_timers();
    }
