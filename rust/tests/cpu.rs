...
use chip8::cpu::{
    Chip8, Fault, Quirks, FONT, FONT_CHAR_SIZE, MAX_PROGRAM_SIZE, PROGRAM_START, SCREEN_HEIGHT,
    SCREEN_WIDTH,
};
...
fn machine(opcodes: &[u16]) -> Chip8 {
    let mut chip8 = Chip8::with_seed(0x1234_5678_9ABC_DEF0);
    chip8
        .load(&ops(opcodes))
...
fn step(chip8: &mut Chip8) {
    chip8.step().expect("no fault");
}

fn run(chip8: &mut Chip8, cycles: u32) {
    chip8.step_many(cycles).expect("no fault");
}

/// Counts the lit pixels on the screen.
fn lit_pixels(chip8: &Chip8) -> usize {
    chip8.framebuffer().iter().filter(|&&lit| lit).count()
}

#[test]
fn starts_at_the_program_entry_point_with_the_font_in_place() {
    let chip8 = machine(&[0x1200]);

    assert_eq!(chip8.pc(), PROGRAM_START);
    assert_eq!(&chip8.memory()[..FONT.len()], &FONT);
    assert_eq!(chip8.memory()[PROGRAM_START as usize], 0x12);
}
...
fn rejects_a_program_that_does_not_fit() {
    ...
    assert!(chip8.load(&vec![0; MAX_PROGRAM_SIZE]).is_ok());
    assert!(chip8.load(&vec![0; MAX_PROGRAM_SIZE + 1]).is_err());
}

#[test]
fn reset_restores_the_loaded_program() {
    // LD V0, 0x42 ; CLS
    let mut chip8 = machine(&[0x6042, 0x00E0]);
    run(&mut chip8, 2);

    assert_eq!(chip8.register(0), 0x42);
    assert_eq!(chip8.pc(), PROGRAM_START + 4);

    chip8.reset();

    assert_eq!(chip8.register(0), 0);
    assert_eq!(chip8.pc(), PROGRAM_START);
    assert_eq!(chip8.memory()[PROGRAM_START as usize], 0x60);
}
...
fn drawing_a_blank_sprite_does_not_report_a_collision() {
    ...
    let mut chip8 = machine(&[0xA300, 0xD001]);
    chip8.write_memory(0x300, 0x00);

    run(&mut chip8, 2);

    assert_eq!(lit_pixels(&chip8), 0);
    assert_eq!(chip8.register(0xF), 0, "a blank sprite must not collide");
}

#[test]
fn collision_is_only_reported_when_a_lit_pixel_is_erased() {
    // LD I, 0x300 ; DRW V0, V0, 1 ; DRW V0, V0, 1
    let mut chip8 = machine(&[0xA300, 0xD001, 0xD001]);
    chip8.write_memory(0x300, 0b1000_0000);

    run(&mut chip8, 2);
    assert!(chip8.pixel(0, 0));
    assert_eq!(chip8.register(0xF), 0, "the screen was empty");
    ...
    assert_eq!(chip8.register(0xF), 1, "erasing a pixel is a collision");
}
...
fn collision_is_latched_for_the_whole_sprite() {
    ...
    let mut chip8 = machine(&[0xA300, 0xD001, 0xA301, 0xD002]);
    chip8.write_memory(0x300, 0b1000_0000);
    chip8.write_memory(0x301, 0b1000_0000);
    chip8.write_memory(0x302, 0b0100_0000);

    run(&mut chip8, 4);

    assert!(!chip8.pixel(0, 0), "the overlapping pixel was erased");
    assert!(chip8.pixel(1, 1), "the second row was drawn");
...
fn sprites_are_clipped_at_the_screen_edge_by_default() {
    ...
    let mut chip8 = machine(&[0x603F, 0x611F, 0xA300, 0xD012]);
    chip8.write_memory(0x300, 0xFF);
    chip8.write_memory(0x301, 0xFF);

    run(&mut chip8, 4);

    assert!(chip8.pixel(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1));
    assert_eq!(
        lit_pixels(&chip8),
        1,
    ...
    assert!(
        !chip8.pixel(0, SCREEN_HEIGHT - 1),
        "it must not wrap horizontally"
    );
    assert!(
        !chip8.pixel(SCREEN_WIDTH - 1, 0),
...
fn sprites_wrap_when_the_quirk_is_turned_off() {
    ...
    assert_eq!(lit_pixels(&chip8), 16);
    assert!(chip8.pixel(SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1));
    assert!(chip8.pixel(0, SCREEN_HEIGHT - 1), "wrapped horizontally");
    assert!(chip8.pixel(SCREEN_WIDTH - 1, 0), "wrapped vertically");
}

#[test]
fn the_starting_position_of_a_sprite_always_wraps() {
    // (65, 33) is the same as (1, 1) on a 64x32 screen.
    let mut chip8 = machine(&[0x6041, 0x6121, 0xA300, 0xD011]);
    chip8.write_memory(0x300, 0b1000_0000);

    run(&mut chip8, 4);

    assert!(chip8.pixel(1, 1));
    assert_eq!(lit_pixels(&chip8), 1);
}

#[test]
fn add_wraps_around_and_reports_the_carry() {
    // LD V0, 0xFF ; LD V1, 0x02 ; ADD V0, V1
    let mut chip8 = machine(&[0x60FF, 0x6102, 0x8014]);
    run(&mut chip8, 3);

    assert_eq!(chip8.register(0), 0x01);
    assert_eq!(chip8.register(0xF), 1);
}
...
fn subtract_wraps_around_instead_of_going_negative() {
    ...
    let mut chip8 = machine(&[0x6000, 0x6101, 0x8015]);
    run(&mut chip8, 3);

    assert_eq!(chip8.register(0), 0xFF);
    assert_eq!(chip8.register(0xF), 0, "VF is NOT borrow");
}

#[test]
fn reverse_subtract_wraps_around_too() {
    // LD V0, 0x05 ; LD V1, 0x03 ; SUBN V0, V1  (V0 = V1 - V0)
    let mut chip8 = machine(&[0x6005, 0x6103, 0x8017]);
    run(&mut chip8, 3);

    assert_eq!(chip8.register(0), 0xFE);
...
fn shift_left_keeps_the_result_inside_a_byte() {
    ...
    // LD V0, 0xFF ; SHL V0
    let mut chip8 = machine(&[0x60FF, 0x800E]);
    run(&mut chip8, 2);

    assert_eq!(chip8.register(0), 0xFE);
    assert_eq!(chip8.register(0xF), 1, "the bit that fell off the top");
}

#[test]
fn shift_right_reports_the_bit_that_fell_off() {
    // LD V0, 0x03 ; SHR V0
    let mut chip8 = machine(&[0x6003, 0x8006]);
    run(&mut chip8, 2);
...
fn add_byte_wraps_without_touching_the_flag() {
    ...
    let mut chip8 = machine(&[0x6F07, 0x60FF, 0x7002]);
    run(&mut chip8, 3);

    assert_eq!(chip8.register(0), 0x01);
    assert_eq!(chip8.register(0xF), 0x07, "7xkk never sets the carry flag");
}

#[test]
fn arithmetic_into_vf_keeps_the_flag_not_the_result() {
    // LD VF, 0x01 ; LD V1, 0x01 ; ADD VF, V1 -> VF holds the carry, which is 0.
    let mut chip8 = machine(&[0x6F01, 0x6101, 0x8F14]);
    run(&mut chip8, 3);
...
fn random_can_produce_every_byte_including_255() {
    ...
    // at 254.
    let mut chip8 = machine(&[0xC0FF, 0x1200]);
    let mut seen = [false; 256];

    for _ in 0..20_000 {
        run(&mut chip8, 2);
        seen[usize::from(chip8.register(0))] = true;
    }
...
fn random_is_masked_by_the_operand() {
    let mut chip8 = machine(&[0xC00F, 0x1200]);

    for _ in 0..1_000 {
        run(&mut chip8, 2);
        assert_eq!(chip8.register(0) & 0xF0, 0);
    }
}
...
fn an_opcode_of_zero_is_a_fault_rather_than_a_silent_stop() {
    ...
    let mut chip8 = machine(&[0x1202]);
    step(&mut chip8);

    let fault = chip8.step().expect_err("0x0000 is not an instruction");
...
fn timers_stop_at_zero() {
    ...
    let mut chip8 = machine(&[0x6002, 0xF015, 0xF018]);
    run(&mut chip8, 3);
...
fn call_and_return_walk_the_stack() {
    ...
    let mut chip8 = machine(&[0x2204, 0x1200, 0x00EE]);
...
fn too_many_calls_are_a_fault() {
    ...
    let mut chip8 = machine(&[0x2200]);

    let fault = chip8.step_many(20).expect_err("the stack is 16 deep");
...
fn returning_without_calling_is_a_fault() {
    let mut chip8 = machine(&[0x00EE]);

    assert_eq!(chip8.step(), Err(Fault::StackUnderflow { pc: 0x200 }));
}

#[test]
fn skips_jump_over_one_instruction() {
    // LD V0, 5 ; SE V0, 5 ; LD V1, 1 ; LD V1, 2
    let mut chip8 = machine(&[0x6005, 0x3005, 0x6101, 0x6102]);
    run(&mut chip8, 3);

    assert_eq!(chip8.register(1), 2, "the LD V1, 1 was skipped");
}

#[test]
fn jump_with_offset_uses_v0() {
    // LD V0, 4 ; JP V0, 0x300
    let mut chip8 = machine(&[0x6004, 0xB300]);
    run(&mut chip8, 2);

    assert_eq!(chip8.pc(), 0x304);
}
...
fn bcd_splits_a_byte_into_three_digits() {
    ...
    let mut chip8 = machine(&[0x60FE, 0xA300, 0xF033]);
    run(&mut chip8, 3);

    assert_eq!(&chip8.memory()[0x300..0x303], &[2, 5, 4]);
}
...
fn registers_round_trip_through_memory() {
    ...
    assert_eq!(&chip8.memory()[0x300..0x303], &[0xAA, 0xBB, 0xCC]);
    assert_eq!(chip8.register(0), 0xAA);
    assert_eq!(chip8.register(1), 0xBB);
    assert_eq!(chip8.register(2), 0xCC);
    assert_eq!(chip8.index(), 0x300, "I is left alone by default");
}

#[test]
fn load_and_store_can_move_the_index_register() {
    let mut chip8 = machine(&[0xA300, 0xF255]);
    chip8.set_quirks(Quirks {
...
fn writing_past_the_end_of_memory_is_a_fault() {
    ...
    let mut chip8 = machine(&[0xAFFF, 0xFF55]);
    step(&mut chip8);

    let fault = chip8.step().expect_err("the write runs past 4 KiB");
...
fn font_sprites_are_addressable() {
    ...
    let mut chip8 = machine(&[0x600A, 0xF029]);
    run(&mut chip8, 2);

    let address = chip8.index();
    assert_eq!(address, 0xA * FONT_CHAR_SIZE);

    let expected = &FONT[usize::from(address)..usize::from(address + FONT_CHAR_SIZE)];
    assert_eq!(
        &chip8.memory()[address as usize..(address + FONT_CHAR_SIZE) as usize],
        expected
    );
}

#[test]
fn the_font_digit_is_masked_to_a_nibble() {
    // A register holding 0xFA must still land on the sprite for 0xA.
    let mut chip8 = machine(&[0x60FA, 0xF029]);
    run(&mut chip8, 2);

    assert_eq!(chip8.index(), 0xA * FONT_CHAR_SIZE);
}

#[test]
fn clear_screen_empties_the_framebuffer() {
    let mut chip8 = machine(&[0xA300, 0xD005, 0x00E0]);
    chip8.write_memory(0x300, 0xFF);

    run(&mut chip8, 2);
    assert!(lit_pixels(&chip8) > 0);
...
fn the_redraw_flag_follows_the_framebuffer() {
    ...
    // draw) ; LD V1, 1
    let mut chip8 = machine(&[0xD000, 0x6101]);
    assert!(
        chip8.take_redraw(),
    ...
    step(&mut chip8);
    assert!(!chip8.take_redraw(), "LD does not touch the screen");
}
...
fn skip_instructions_read_the_keypad() {
    ...
    let mut chip8 = machine(&[0x6007, 0xE09E, 0x1208, 0x120A]);
    chip8.keypad_mut().set_pressed(0x7, true);
    run(&mut chip8, 3);
    ...
    chip8.reset();
    chip8.keypad_mut().set_pressed(0x7, false);
    run(&mut chip8, 3);
...
fn waiting_for_a_key_blocks_until_it_is_released() {
    // LD V0, K ; CLS
    let mut chip8 = machine(&[0xF00A, 0x00E0]);

    step(&mut chip8);
    assert_eq!(chip8.pc(), 0x200, "still on the same instruction");
    assert!(chip8.is_awaiting_key());

    chip8.keypad_mut().set_pressed(0xB, true);
    step(&mut chip8);
    assert_eq!(chip8.pc(), 0x200, "a key that is still held does not count");

    chip8.keypad_mut().set_pressed(0xB, false);
    step(&mut chip8);

    assert_eq!(chip8.register(0), 0xB);
...
fn keys_released_before_the_wait_started_are_ignored() {
    ...
    chip8.keypad_mut().set_pressed(0x3, true);
    chip8.keypad_mut().set_pressed(0x3, false);

    step(&mut chip8);
    assert!(
        chip8.is_awaiting_key(),
...
fn the_keypad_covers_all_sixteen_keys_exactly_once() {
    use chip8::keypad::{Keypad, KEY_BINDINGS, KEY_COUNT};
