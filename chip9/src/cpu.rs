//! The CHIP-9 virtual machine: memory, registers, timers and the instruction
...
/// Where the built-in font lives.
pub const FONT_START: u16 = 0x000;

/// Bytes per font character.
pub const FONT_CHAR_SIZE: u16 = 5;

/// How many characters the font holds: the ten digits, then `A` through `Z`.
///
/// CHIP-8 stopped at sixteen, which is exactly the hexadecimal digits `0`–`F`.
/// CHIP-9 carries on through the alphabet, so a character is a base 36 digit
/// and everything below 16 means what it always did.
pub const FONT_CHAR_COUNT: u8 = 36;
...
/// How many bytes fit on the data stack that `PUSH` and `POP` work on.
///
/// This is separate from the call stack above: `CALL` and `RET` cannot reach
/// it, and it cannot unbalance a return.
pub const DATA_STACK_SIZE: usize = 64;
...
/// Sprites for `0`–`9` and `A`–`Z`, four pixels wide and five rows tall.
///
/// The first sixteen are byte for byte the CHIP-8 font. The rest were drawn to
/// match them, which in four columns means a few unavoidable collisions: `O`
/// is the same shape as `0`, `S` the same as `5`, and `M` and `W` are each
/// other upside down.
#[rustfmt::skip]
pub const FONT: [u8; FONT_CHAR_SIZE as usize * FONT_CHAR_COUNT as usize] = [
    ...
    0x90, 0x90, 0x90, 0xF0, 0xF0, // W
    0x90, 0x90, 0x60, 0x90, 0x90, // X
    0x90, 0x90, 0x60, 0x40, 0x40, // Y
    0xF0, 0x20, 0x40, 0x80, 0xF0, // Z
];
...
pub enum Fault {
    ...
    /// `PUSH` was executed with a full data stack.
    DataStackOverflow { pc: u16 },
    /// `POP` was executed with an empty data stack.
    DataStackUnderflow { pc: u16 },
...
impl fmt::Display for Fault {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            Self::DataStackOverflow { pc } => write!(f, "data stack overflow at {pc:#05X}"),
            Self::DataStackUnderflow { pc } => write!(f, "data stack underflow at {pc:#05X}"),
            Self::DivideByZero { pc } => write!(f, "divide by zero at {pc:#05X}"),
...
impl Fault {
    ...
    pub fn hint(self) -> &'static str {
        match self {
            ...
            Self::DataStackOverflow { .. } => "more than 64 bytes pushed without popping them",
            Self::DataStackUnderflow { .. } => "a pop without a matching push",
            Self::DivideByZero { .. } => "DIV was given a divisor of zero",
...
pub struct Chip9 {
    ...
    redraw: bool,
    /// The bytes handed to [`Chip9::load`], kept so [`Chip9::reset`] can reload.
    program: Vec<u8>,
    rng: u64,
}
...
impl Chip9 {
    ...
    /// Clears the machine and reloads the program passed to [`Chip9::load`].
    ///
    ...
    /// How many bytes are on the data stack.
    #[must_use]
    pub fn data_stack_depth(&self) -> usize {
        self.data_sp
    }

    /// The bytes on the data stack, oldest first.
    #[must_use]
    ...
    fn execute(&mut self, opcode: u16, pc: u16) -> Result<(), Fault> {
        ...
        match (u, x, y, n) {
            ...
            // 8xy8 - MUL Vx, Vy - Set Vx = the low byte of Vx * Vy, VF = the high byte.
            (0x8, _, _, 0x8) => {
                let product = u16::from(self.v[x]) * u16::from(self.v[y]);
                self.set_result(x, product as u8, (product >> 8) as u8);
            }
            // 8xy9 - DIV Vx, Vy - Set Vx = Vx / Vy, VF = the remainder.
            (0x8, _, _, 0x9) => {
                let (dividend, divisor) = (self.v[x], self.v[y]);
                if divisor == 0 {
                    return Err(Fault::DivideByZero { pc });
                }
                self.set_result(x, dividend / divisor, dividend % divisor);
            }
            ...
            // Fx01 - PUSH Vx - Push Vx onto the data stack.
            (0xF, _, 0x0, 0x1) => {
                if self.data_sp >= DATA_STACK_SIZE {
                    return Err(Fault::DataStackOverflow { pc });
                }
                self.data_stack[self.data_sp] = self.v[x];
                self.data_sp += 1;
            }
            // Fx02 - POP Vx - Take the top of the data stack off into Vx.
            (0xF, _, 0x0, 0x2) => {
                self.data_sp = self
                    .data_sp
                    .checked_sub(1)
                    .ok_or(Fault::DataStackUnderflow { pc })?;
                self.v[x] = self.data_stack[self.data_sp];
            }
            // Fx03 - LD Vx, SP - Set Vx to how many bytes are on the data stack.
            (0xF, _, 0x0, 0x3) => self.v[x] = self.data_sp as u8,
            ...
            // Fx29 - LD F, Vx - Set I to the sprite for the character in Vx.
            (0xF, _, 0x2, 0x9) => {
                let character = self.v[x] % FONT_CHAR_COUNT;
                self.i = FONT_START + u16::from(character) * FONT_CHAR_SIZE;
            }
