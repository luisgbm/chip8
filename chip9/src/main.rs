...
fn main() -> Result<()> {
    ...
    let window = video_subsystem
        .window("CHIP-9", WINDOW_WIDTH as u32, WINDOW_HEIGHT as u32)
        .position_centered()
        .build()
...
impl Session {
    ...
    fn from_file(path: &Path) -> Result<Self> {
        ...
        Self::new(
            ...
            &rom,
            chip9::programs::DEFAULT_CYCLES_PER_FRAME,
        )
    }
    ...
    fn update(&mut self) {
        ...
        if let Err(fault) = self.chip9.step_many(self.cycles_per_frame) {
        ...
        match &mut self.state {
            ...
            State::Session(session) => match *event {
                ...
                Event::KeyDown {
                    ...
                    _ => {
                        if let Some(key) = Keypad::key_for(keycode) {
                            session.chip9.keypad_mut().set_pressed(key, true);
                        }
                    }
                },
                Event::KeyUp {
                    ...
                    if let Some(key) = Keypad::key_for(keycode) {
                        session.chip9.keypad_mut().set_pressed(key, false);
                    }
                }
                ...
                Event::Window {
                    ...
                    session.chip9.keypad_mut().reset();
                }
                _ => {}
            },
        }
    ...
    fn update(&mut self) {
        match &mut self.state {
            ...
            State::Session(session) => {
                session.update();
                self.beeper
                    .set_playing(!session.is_stopped() && session.chip9.is_beeping());
            }
        }
    }
...
fn render_session(video: &mut Video, session: &mut Session) {
    if session.chip9.take_redraw() || session.needs_full_redraw {
        video.draw_chip9_screen(
            session.chip9.framebuffer(),
            0,
            0,
            PIXEL_SIZE,
            theme::SCREEN_ON,
    ...
    } else if session.chip9.is_awaiting_key() {
...
mod tests {
    ...
    fn a_builtin_session_starts_ready_to_run() {
        ...
        assert_eq!(session.chip9.program(), pong().rom);
    ...
    fn a_session_from_a_file_is_named_after_it() {
        ...
        assert_eq!(session.chip9.program().len(), 1296);
    }
    ...
    fn a_file_that_does_not_fit_in_memory_is_refused() {
        let path = std::env::temp_dir().join("chip9-too-large.ch8");
    ...
    fn a_stopped_session_does_not_run_and_can_be_restarted() {
        ...
        let pc = session.chip9.pc();
        session.update();
        assert_eq!(session.chip9.pc(), pc, "a stopped program must not advance");
