//! Turns C9 source into a flat list of tokens.
...
pub fn tokenize(source: &str) -> Result<Vec<Token>, LexError> {
    ...
    while index < bytes.len() {
        ...
        if byte == b'\'' {
            let (value, length) = character(&source[index..], line)?;
            tokens.push(Token {
                kind: TokenKind::Number(value),
                line,
            });
...
/// `'A'` is the index of that glyph in the font, which is what makes a letter
/// a number the machine can draw. Digits and letters share one run: `'0'` is
/// 0 and `'Z'` is 35.
fn character(text: &str, line: usize) -> Result<(i32, usize), LexError> {
    let mut characters = text.char_indices().skip(1);

    let Some((_, glyph)) = characters.next() else {
        return Err(LexError {
            line,
            message: "a character literal needs a character in it".into(),
        });
    };

    let Some((end, '\'')) = characters.next() else {
        return Err(LexError {
            line,
            message: format!("`'{glyph}` needs a closing quote"),
        });
    };

    let Some(value) = glyph.to_digit(36) else {
        return Err(LexError {
            line,
            message: format!("`{glyph}` is not in the font, which holds 0 to 9 and A to Z"),
        });
    };
