//! A small two pass assembler for the CHIP-9 instruction set.
//!
...
//! ```
//!
//! Numbers may be written as `31`, `0x1F`, `$1F`, `#1F` or `'Z'`, and any
//! value may be a sum or difference of numbers, constants and labels. A
//! character literal is the font's index for that character, so `'A'` is 10
//! and `'Z'` is 35.
//!
//! # What CHIP-9 adds
//!
//! The 35 opcodes of the original CHIP-8 all assemble to the same bytes they
//! always did, so any CHIP-8 program is a CHIP-9 program. On top of them:
//!
//! ```text
//! MUL Vx, Vy                   8xy8   Vx = the low byte, VF = the high byte
//! DIV Vx, Vy                   8xy9   Vx = the quotient, VF = the remainder
//! PUSH Vx                      Fx01   push a byte onto the data stack
//! POP Vx                       Fx02   take one off again
//! LD Vx, SP                    Fx03   how deep the data stack is
//! ```
//!
//! `LD F, Vx` also reaches the whole alphabet rather than stopping at `F`.
//!
//! `SHR Vx` and `SHL Vx` assemble to `8xx6` and `8xxE`, which do the same
//! thing whether or not the interpreter has the "shift in place" quirk.
...
/// `31`, `0x1F`, `$1F`, `#1F`, or a character literal like `'A'`.
///
/// A character literal is the number the font knows it by, so `'0'` is 0,
/// `'A'` is 10 and `'Z'` is 35 — a base 36 digit, which is exactly what
/// `LD F, Vx` wants.
fn parse_number(token: &str) -> Option<i32> {
    if let Some(character) = parse_character(token) {
...
/// `'A'`, as the font's index for that character.
fn parse_character(token: &str) -> Option<i32> {
    ...
    character.to_digit(36).map(|digit| digit as i32)
}
...
fn encode(statement: &Statement, symbols: &HashMap<String, i32>) -> Result<u16, AsmError> {
    ...
    match statement.mnemonic.as_str() {
        ...
        "PUSH" => {
            count(1)?;
            Ok(0xF001 | register(0)? << 8)
        }
        "POP" => {
            count(1)?;
            Ok(0xF002 | register(0)? << 8)
        }
        "SHR" | "SHL" => {
...
mod tests {
    ...
    #[test]
    fn chip9_adds_multiply_divide_and_a_data_stack() {
        assert_eq!(opcode("MUL V5, V6"), 0x8568);
        assert_eq!(opcode("DIV V5, V6"), 0x8569);
        assert_eq!(opcode("PUSH VA"), 0xFA01);
        assert_eq!(opcode("POP VA"), 0xFA02);
        assert_eq!(opcode("LD VA, SP"), 0xFA03);
    }

    #[test]
    fn a_character_literal_is_its_index_in_the_font() {
        assert_eq!(opcode("LD V0, '0'"), 0x6000);
        assert_eq!(opcode("LD V0, '9'"), 0x6009);
        assert_eq!(opcode("LD V0, 'A'"), 0x600A);
        assert_eq!(opcode("LD V0, 'F'"), 0x600F);
        assert_eq!(opcode("LD V0, 'Z'"), 0x6023);
        // Lowercase reads as the same character, and arithmetic still works.
        assert_eq!(opcode("LD V0, 'z'"), 0x6023);
        assert_eq!(opcode("LD V0, 'A' + 1"), 0x600B);
        assert_eq!(rom("DB 'H', 'I'"), vec![0x11, 0x12]);
    }
