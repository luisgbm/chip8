...
No prior assembly experience is assumed. CHIP-9 is a good first machine: it has
sixteen registers, forty instructions, and you can hold all of it in your head
at once.

CHIP-9 is CHIP-8 with five instructions added — multiply, divide, and a data
stack — and a font that runs to `Z` instead of stopping at `F`. Everything a
CHIP-8 program did, it still does, byte for byte. The additions are marked
**new** where they come up.
...
Everything a CHIP-9 program can touch:
...
| **Call stack** | Sixteen entries, used only by `CALL` and `RET`. You cannot read it. |
| **Data stack** | **New.** Sixty-four bytes, reached with `PUSH` and `POP`. This is where you keep a value when you have run out of registers. |
...
There are no signed numbers, no memory to speak of, and no way to print.
Everything is sprites.

### The built in font

The interpreter keeps a sprite for each of the thirty six digits of base thirty
six in low memory: `0` to `9`, then `A` to `Z`. Each is five rows tall and four
columns wide. You never need to know where they are, because `LD F, Vx` points
`I` at the glyph for the digit in `Vx`:
...
The first sixteen glyphs are the CHIP-8 ones, byte for byte, which is why every
CHIP-8 program still draws exactly what it used to. The other twenty are new,
and they are what lets [`hello.c9`](hello.c9) write `HELLO WORLD` without
drawing a single sprite of its own.

The assembler will take a letter in quotes and give you its index, so you do not
have to count:

```
LD  V0, 'H'     ; the same as LD V0, 17
LD  F, V0
```

Four columns is not much room for an alphabet. `O` comes out the same shape as
`0`, `S` the same as `5`, and `W` is `M` upside down. That is the price of the
width, and it is why `HELLO WORLD` reads with a zero in it.

This tutorial still spells `ABC123`, because that program is also written in C9
and in the CHIP-8 port next door, and the three are checked against each other.
...
stop:
    JP   stop                   ; a CHIP-9 program has nowhere to return to
...
in any CHIP-9 program by a wide margin.
...
mistake in CHIP-9, so read every skip out loud.
...
A CHIP-9 program has nothing to return to, so it has to end in a loop that jumps
...
- A character in single quotes is its base thirty six value, which is its index
  in the font: `'0'` is 0, `'A'` is 10, `'Z'` is 35.
...
All 35 opcodes of the original CHIP-8, unchanged, and the five CHIP-9 adds.
...
| `8xy8` | `MUL Vx, Vy` | **New.** `Vx` \*= `Vy`, and `VF` = the high byte of the product |
| `8xy9` | `DIV Vx, Vy` | **New.** `Vx` /= `Vy`, and `VF` = the remainder. Dividing by zero halts the machine |
...
| `Fx01` | `PUSH Vx` | **New.** Put `Vx` on the data stack. Overflowing 64 bytes halts the machine |
| `Fx02` | `POP Vx` | **New.** Take the top of the data stack into `Vx`. Popping an empty stack halts the machine |
| `Fx03` | `LD Vx, SP` | **New.** `Vx` = how many bytes are on the data stack |
...
| `Fx1E` | `ADD I, Vx` | `I` += `Vx` |
| `Fx29` | `LD F, Vx` | `I` = the font glyph for the base thirty six digit in `Vx` |
...
`MUL` and `DIV` are the one place a byte is not enough. `MUL` keeps the low byte
of the product in `Vx` and puts the high byte in `VF`, so `LD V0, 20 / LD V1, 20
/ MUL V0, V1` leaves 144 in `V0` and 1 in `VF`. `DIV` puts the remainder in
`VF`, which is how you get a modulo out of it:

```asm
    LD  V0, 47
    LD  V1, 10
    DIV V0, V1      ; V0 = 4, VF = 7
    LD  V2, VF      ; and now V2 is the remainder
```

The data stack is separate from the one `CALL` uses, and it is the piece CHIP-8
was missing: somewhere to put a value that is not a register. It is what makes a
subroutine able to call itself.
...
**`VF` is not yours.** `ADD Vx, Vy`, `SUB`, `SUBN`, `MUL`, `DIV`, `SHR`, `SHL`
and `DRW` all write to it. Read it in the instruction straight after the one
that set it, and never store anything there.
...
a sprite around, and hand drawn letter sprites for the words. It was written for
CHIP-8, where the font stopped at `F`; on CHIP-9 those sprites could come out of
the font instead, and [`hello.c9`](hello.c9) shows what that looks like.

Then read [`leap.c9`](leap.c9), which is the same game written in
[C9](LANGUAGE.md), the C-like language that compiles to these instructions.
...
[`times.c9`](times.c9) is the shorter tour of what CHIP-9 added: `MUL`, `DIV`
